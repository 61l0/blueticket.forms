...
class blueticket_forms
{
            ...
            foreach ($this->inner_table_instance as $inst_name => $field)
            {
                if (isset($this->result_row[$field]))
                {
                    ...
                    $this->nested_rendered[$inst_name] =
                        '<div class="blueticket_forms-nested-container blueticket_forms-container"><div class="blueticket_forms-ajax" id="blueticket_forms-ajax-' . base_convert(rand(), 10,
    ...
    public function _export_vars()
    {
        ...
        if ($this->instance_count == 1 && !$this->ajax_request)
        {
            ...
            {
                foreach ($_SESSION['blueticket_forms_session'] as $s_key => $s_val)
    ...
    protected function lang($text = '')
    {
        ...
        return htmlspecialchars((isset($this->lang_arr[$langtext]) ? $this->lang_arr[$langtext] : $text), ENT_QUOTES,
            blueticket_forms_config::$mbencoding);
    }
        ...
        switch ($attr['data-type'])
        {
            ...
            case 'timestamp':
                if (!isset($fieldlist['date']) or $field == $this->column)
                {
                    ...
                    $fieldlist['date'] .= $this->single_tag('input', 'blueticket_forms-datepicker-from ' . $class . ' ' . $this->theme_config('search_from'),
                        $attr);
                    $attr['name'] = 'phrase][to';
                    $attr['value'] = (isset($phrase['to']) && $field == $this->column) ? $phrase['to'] : '';
                    $fieldlist['date'] .= $this->single_tag('input', 'blueticket_forms-datepicker-to ' . $class . ' ' . $this->theme_config('search_to'),
                        $attr);
                }
                break;
            case 'select':
